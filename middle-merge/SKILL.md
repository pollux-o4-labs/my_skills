---
name: middle-merge
description: middle-merge git 워크플로 상세 — 서브에이전트 병렬 작업의 worktree isolation 기준, 검증 책임 분배(sub-agent vs supervisor), 4-layer PR 검증 chain, branch 구조·수명, 머지 옵션(squash/merge), issue→branch 자동 세팅. Use when repo 루트 CLAUDE.md 워크플로 마커가 `middle-merge` 이고 서브에이전트로 브랜치 작업을 오케스트레이션할 때. git-workflow-select 가 middle-merge 선택 시 이 스킬로 라우팅. 범용 서브에이전트 규율은 `efficient-subagent`.
