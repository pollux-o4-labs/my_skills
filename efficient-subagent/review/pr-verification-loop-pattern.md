...
| 상황 | 검증 주체 |
|---|---|
| pytest / mock 으로 충분히 잡히는 영역 (순수 로직, 내부 helper, type fix, import, mock-able 통합) | **AI end-to-end** — sub-agent 자체 test + supervisor 회귀 확인. 사용자 통보만 |
...
4. 사용자: root cwd 그대로 — 파일 이미 반영됨, 즉시 검증
5. 사용자: mise run butler-run  (봇 재기동 필요하면)
```

사용자 손: **gh pr checkout 불필요**.

---

## branch switch 시 사용자 체크리스트

Claude 가 `git switch -c` 또는 `git switch` 할 때 사용자에게 한 줄 알림:
...
_이 파일은 review draft — 사용자 검토 후 SKILL.md 및 CLAUDE.md 에 반영 여부 결정._
