...
2. **Actually open the source.** Find where the unit, boundaries, and naming are already defined and read them — already-written items (content/data/samples), the current schema or config, naming conventions, design records. Confirm by grep/opening, not by "there's probably…".
