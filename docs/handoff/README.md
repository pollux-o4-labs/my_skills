...
- git-workflow-gate-python-silent-fail — `git-workflow-select/hooks/git-workflow-gate.sh` 가 `python` 만 찾고 `python3` 는 안 찾아, 데비안·우분투 기본 환경(python3 만 설치)에서 마커 없는 repo 의 브랜치 생성을 무음 통과시킴을 실측 확인(근거: `docs/history/B-worktree-cleanup-gate.md` "인터프리터" 절) — 미수정, 별도 착수 필요 — 2026-07-16
