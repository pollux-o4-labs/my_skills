...
- Re-measure after editing by running `python3 ./skillify-session-lessons/scripts/measure_skill.py <skill-dir>`, and report as your final text: what changed, plus the script's output. Non-zero exit = over a cap; fix it or state why it stands. Do not hand-count — the script reads the caps from the standard, you would be guessing at both.
