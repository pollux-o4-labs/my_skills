...
- Name prefix `AIL-`; frontmatter `version` (semver, bump on every edit), `metadata.provenance: AIL`. (No `metadata.platforms` — nothing consumes it; sync is by provenance + manifest. Strip it on edit; don't add it.)
